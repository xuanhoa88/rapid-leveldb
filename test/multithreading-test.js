...
test('check multithreading flag works as expected', async function (t) {
  ...
  const db1 = new LevelDB(location);
  await db1.open();
  ...
  const db3 = new LevelDB(location, { multithreading: true });
  await db3.open();
  t.is(db3.location, location);
  const db4 = new LevelDB(location);
  try {
  ...
  const db5 = new LevelDB(location);
...
test('allow multi-threading by same process', async function (t) {
  try {
    const location = tempy.directory();
    const db = new LevelDB(location, { multithreading: true });
    await db.open();
