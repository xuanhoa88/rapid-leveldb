...
class NoopLevel extends AbstractLevel {
  constructor(...args) {
    super({ encodings: { utf8: true, buffer: true, view: true } }, ...args);
  }
}
...
test('sublevel name and options', function (t) {
  ...
  t.test('invalid sublevel prefix', function (t) {
    t.throws(
      () => new NoopLevel().sublevel('foo\x05'),
      err => err.code === 'LEVEL_INVALID_PREFIX'
    );
    t.throws(
      () => new NoopLevel().sublevel('foo\xff'),
      err => err.code === 'LEVEL_INVALID_PREFIX'
    );
    t.throws(
      () => new NoopLevel().sublevel(['ok', 'foo\xff']),
      err => err.code === 'LEVEL_INVALID_PREFIX'
    );
    t.throws(
      () => new NoopLevel().sublevel('foo!', { separator: '@' }),
      err => err.code === 'LEVEL_INVALID_PREFIX'
    );
    t.throws(
      () => new NoopLevel().sublevel(['ok', 'foo!'], { separator: '@' }),
      err => err.code === 'LEVEL_INVALID_PREFIX'
    );
    t.end();
  });
  ...
  t.test('doubly nested sublevel has correct prefix', async function (t) {
    ...
    t.same(keys.sort(), ['!1!!2!!3!c', '!1!!2!b', '!1!a']);
  });
...
test('opening & closing sublevel', function (t) {
  ...
  t.test('can close db and sublevel once opened', async function () {
    const db = new NoopLevel();
    await db.open();
    const sub = db.sublevel('test');
    await sub.open();
...
test('sublevel operations are prefixed', function (t) {
  ...
  for (const [mode, def] of [
    ...
    for (const deferred of [false, true]) {
      t.test(
        `sublevel ${mode}.seek() target is prefixed (default implementation: ${def}, deferred: ${deferred})`,
        async function (t) {
          t.plan(2);
...
test('sublevel encodings', function (t) {
  ...
  t.test('sublevel indirectly supports transcoded encoding', async function (t) {
    ...
    class MockLevel extends AbstractLevel {
      async _put(key, value, { keyEncoding, valueEncoding }) {
        t.same(
          { key, value, keyEncoding, valueEncoding },
          {
  ...
  for (const [mode, def] of [
    ...
    t.test(
      `unfixing sublevel.${mode}() Buffer keys (default implementation: ${def})`,
      async function (t) {
        t.plan(3);
    ...
    t.test(
      `unfixing sublevel.${mode}() Uint8Array keys (default implementation: ${def})`,
      async function (t) {
        t.plan(3);
    ...
    mode === 'values' ||
      t.test(
        `sublevel.${mode}() skips unfixing undefined keys (default implementation: ${def})`,
        async function (t) {
          ...
          t.same(
            await sub[publicMethod]().next(),
            mode === 'iterator' ? [undefined, 'foo'] : undefined
          );
          t.same(
            await sub[publicMethod]().nextv(1),
            mode === 'iterator' ? [[undefined, 'foo']] : [undefined]
          );
          t.same(
            await sub[publicMethod]().all(),
            mode === 'iterator' ? [[undefined, 'foo']] : [undefined]
          );
        }
      );
