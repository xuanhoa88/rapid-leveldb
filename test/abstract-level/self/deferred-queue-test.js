...
supported &&
  test('DeferredQueue does not add operation if given an aborted signal', async function (t) {
...
supported &&
  test('DeferredQueue aborts operation on signal abort', async function (t) {
    ...
    queue.add(
      abortError => {
        calls.push([1, abortError]);
      },
      { signal: ac1.signal }
    );
    queue.add(
      abortError => {
        calls.push([2, abortError]);
      },
      { signal: ac2.signal }
    );
...
supported &&
  test('DeferredQueue calls operation if signal is not aborted', async function (t) {
    const ac1 = new globalThis.AbortController();
    const ac2 = new globalThis.AbortController();
    const queue = new DeferredQueue();
    const calls = [];
