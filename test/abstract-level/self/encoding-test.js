...
for (const deferred of [false, true]) {
  ...
  test(`getMany() takes encoding options (deferred: ${deferred})`, async function (t) {
    ...
    if (!deferred) await db.open();
    t.same(await db.getMany([[1, '2'], 'x'], { keyEncoding: 'json', valueEncoding: 'json' }), [
      123,
      'hi',
    ]);
  });
  ...
  // NOTE: adapted from encoding-down
  deferred ||
    test('chainedBatch.put() and del() encode utf8 key and value', async function (t) {
      t.plan(5);
  ...
  // NOTE: adapted from encoding-down
  deferred ||
    test('chainedBatch.put() and del() take encoding options', async function (t) {
      t.plan(5);
  ...
  test(`clear() does not strip nullish range options (deferred: ${deferred})`, async function (t) {
    ...
    const db1 = mockLevel(
      ...
      utf8Manifest,
      { keyEncoding: nullishEncoding, valueEncoding: nullishEncoding }
    );
