...
exports.setUp = function (test, testCommon) {
  test('iterator setup', async function () {
...
exports.sequence = function (test, testCommon) {
  for (const mode of ['iterator', 'keys', 'values']) {
    test(`${mode}().close() is idempotent`, async function () {
    ...
    for (const method of ['next', 'nextv', 'all']) {
      ...
      if (
        ...
        testCommon.supports.signals &&
        testCommon.supports.signals.iterators
      ) {
        ...
        test(`${mode}().${method}() with non-aborted signal`, async function () {
          const db = testCommon.factory();
...
exports.iterator = function (test, testCommon) {
  test('iterator data setup', function () {
    return db.batch([
  ...
  for (const mode of ['iterator', 'keys', 'values']) {
    ...
    test(`${mode}().all() with values: false`, async function (t) {
      ...
      t.same(
        await db[mode]({ values: false }).all(),
        [
    ...
    test(`${mode}().all() in reverse`, async function (t) {
      t.same(
        await db[mode]({ reverse: true }).all(),
        [
          ['foobatch3', 'bar3'],
          ['foobatch2', 'bar2'],
          ['foobatch1', 'bar1'],
    ...
    test(`${mode}().all() honors limit in reverse`, async function (t) {
      t.same(
        await db[mode]({ limit: 2, reverse: true }).all(),
        [
          ['foobatch3', 'bar3'],
          ['foobatch2', 'bar2'],
        ].map(mapEntry)
  ...
  test('iterator() sorts lexicographically', async function (t) {
    ...
    t.same(await db.iterator({ lte: '' }).all(), [['', 'empty']]);
  ...
  for (const keyEncoding of ['buffer', 'view']) {
    ...
    test(`iterators have byte order (${keyEncoding} encoding)`, async function (t) {
      ...
      t.same(
        (await db.keys().all()).map(k => k[0]),
        [1, 2, 11],
        'order of keys() is ok'
      );
      t.same(
        (await db.iterator().all()).map(e => e[0][0]),
        [1, 2, 11],
        'order of iterator() is ok'
      );
...
exports.tearDown = function (test) {
  test('iterator teardown', async function () {
    return db.close();
  });
};
...
exports.dispose = function (test, testCommon) {
  ...
  Symbol.asyncDispose &&
    test('Symbol.asyncDispose', async function () {
      const db = testCommon.factory();
      await db.open();
