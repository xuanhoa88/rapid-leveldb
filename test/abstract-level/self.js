...
test('batch([]) extensibility', async function (t) {
  ...
  t.same(
    ...
    expectedArray.map(o => ({ ...expectedOptions, ...o })),
    'got expected array argument'
  );
  t.same(spy.getCall(1).args[1], expectedOptions, 'got expected options argument');
...
test('test chained batch() extensibility', async function (t) {
  ...
  t.same(
    spy.getCall(0).args[0][0],
    { keyEncoding: 'utf8', valueEncoding: 'utf8', type: 'put', key: 'foo', value: 'bar' },
    'got expected array argument[0]'
  );
  t.same(
    spy.getCall(0).args[0][1],
    { keyEncoding: 'utf8', type: 'del', key: 'bang' },
  ...
  await test
    .batch()
    .put('foo', 'bar', expectedOptions)
    .del('bang', expectedOptions)
    .write(expectedOptions);
  ...
  t.same(
    spy.getCall(1).args[0][1],
    { keyEncoding: 'utf8', type: 'del', key: 'bang', options: 1 },
    'got expected array argument[1]'
  );
...
test('rangeOptions', function (t) {
  ...
  t.plan(10);
  t.test('setup', async () => db.open());

  t.test('default options', function (t) {
    t.same(
      getRangeOptions(undefined, db.keyEncoding('utf8')),
      {
        reverse: false,
        limit: -1,
      },
  ...
  t.test('set options', function (t) {
    t.same(
      getRangeOptions({ reverse: false, limit: 20 }, db.keyEncoding('utf8')),
      {
        reverse: false,
        limit: 20,
      },
