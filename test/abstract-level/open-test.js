...
exports.open = function (test, testCommon) {
  ...
  test('open() and close(), with empty options', async function () {
    const db = testCommon.factory();
    await db.open({});
    return db.close();
  });
  ...
  test('open(), close() and open() in same tick', function (t) {
    ...
    db.open().then(function () {
      ...
      t.same(
        order,
        ['open event', 'A', 'closed event', 'B', 'open event', 'C'],
        'callback order is the same as call order'
      );
      t.is(db.status, 'open', 'is open');
  ...
  test('passive option is ignored if set in constructor options', async function () {
  ...
  Symbol.asyncDispose &&
    test('Symbol.asyncDispose', async function (t) {
      const db = testCommon.factory();
      await db.open();
