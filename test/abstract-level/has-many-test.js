...
exports.setUp = function (test, testCommon) {
  test('hasMany() setup', async function () {
...
exports.hasMany = function (test, testCommon) {
  ...
  test('simultaneous hasMany()', async function (t) {
    ...
    for (let i = 0; i < 10; ++i) {
      promises.push(
        db.hasMany(['hello']).then(function (values) {
          t.same(values, [true]);
        })
      );
    }
    ...
    for (let i = 0; i < 10; ++i) {
      promises.push(
        db.hasMany(['non-existent']).then(function (values) {
          t.same(values, [false]);
        })
      );
    }
...
exports.tearDown = function (test) {
  test('hasMany() teardown', async function () {
    return db.close();
  });
};
