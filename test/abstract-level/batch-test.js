...
const { illegalKeys, illegalValues, isBuffer } = require('./util');
...
exports.args = function (test) {
  ...
  test('batch([]) with empty operations array and empty options', async function () {
    await db.batch([]);
    await db.batch([], null);
...
exports.batch = function (test, testCommon) {
  ...
  for (const encoding of ['utf8', 'buffer', 'view']) {
    ...
    test(`empty values in batch with ${encoding} valueEncoding`, async function (t) {
      ...
      const expected =
        encoding === 'utf8' ? values[0] : encoding === 'view' ? values[1] : values[2];
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        if (encoding === 'view' && isBuffer(value)) {
...
exports.tearDown = function (test) {
  test('batch([]) teardown', async function () {
    return db.close();
  });
};
