...
exports.setUp = function (test, testCommon) {
  test('chained batch setup', async function () {
...
exports.args = function (test) {
  ...
  test('chained batch.clear() does not throw if empty', async function () {
  ...
  test('chained batch.write() with no operations', async function () {
    return db.batch().write();
  });

  test('chained batch.close() with no operations', async function () {
    return db.batch().close();
  });

  test('chained batch.close() is idempotent', async function () {
    const batch = db.batch();
    await batch.close();
    await batch.close();
      ...
      t.same(operations, [
        {
          ...
          keyEncoding: utf8,
          valueEncoding: json,
          encodedKey: utf8.encode('a'),
          encodedValue: utf8.encode('"a"'),
        },
        {
          type: 'put',
          key: 'b',
          ...
          keyEncoding: utf8,
          valueEncoding: utf8,
          encodedKey: utf8.encode('b'),
          encodedValue: utf8.encode('b'),
        },
        {
          type: 'put',
          key: '"c"',
          ...
          keyEncoding: utf8,
          valueEncoding: utf8,
          encodedKey: utf8.encode('"c"'),
          encodedValue: utf8.encode('c'),
        },
        {
          type: 'del',
          key: 'c',
...
exports.tearDown = function (test) {
  test('chained batch teardown', async function () {
    return db.close();
  });
};
