...
test('test destroy non-existent directory', async function (t) {
  ...
  // TODO: use promise
  const err = rimrafSync(location, { glob: false });
  t.ok(err, 'no error from rimraf()');
...
test('test destroy non-existent parent directory', async function (t) {
  t.plan(2);
...
test('test destroy non leveldb directory', function (t) {
  ...
  mkfiletree.makeTemp('destroy-test', tree, function (err, dir) {
    ...
    LevelDB.destroy(dir).then(function () {
