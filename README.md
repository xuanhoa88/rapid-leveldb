...
<details><summary>Usage from TypeScript requires generic type parameters.</summary>

```ts
// Specify types of keys and values (any, in the case of json).
// The generic type parameters default to LevelDB<string, string>.
const db = new LevelDB<string, any>('./db', { valueEncoding: 'json' })
...
The [`db.iterator([options])`](https://github.com/xuanhoa88/rapid-leveldb#iterator--dbiteratoroptions) method also accepts:
