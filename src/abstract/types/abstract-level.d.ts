  ...
  implements AbstractResource
{
  ...
  // eslint-disable-next-line no-dupe-class-members
  del<K = KDefault>(key: K, options: AbstractDelOptions<K>): Promise<void>;
 ...
 */
export interface AbstractDatabaseOptions<K, V> extends Omit<AbstractOpenOptions, 'passive'> {
  /**
...
export type AbstractBatchOperation<TDatabase, K, V> =
  | AbstractBatchPutOperation<TDatabase, K, V>
  | AbstractBatchDelOperation<TDatabase, K>;
