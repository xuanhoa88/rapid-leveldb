...
class AbstractChainedBatch {
  ...
  constructor(db, options) {
    if (typeof db !== 'object' || db === null) {
      ...
      throw new TypeError(
        `The first argument must be an abstract-level database, received ${hint}`
      );
    }
    ...
    if (enablePrewriteHook) {
      ...
      this.#prewriteBatch = new PrewriteBatch(
        db,
        data[kPrivateOperations],
        data[kPublicOperations]
      );
  ...
  #assertStatus() {
    if (this.#status !== 'open') {
      throw new ModuleError(
        'Batch is not open: cannot change operations after write() or close()',
        {
